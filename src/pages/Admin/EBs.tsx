...
const EBs = () => {
  ...
  const [selected, setSelected] = useState<EB | null>(null);
  const [showForm, setShowForm] = useState(false);

  const fetchEBs = async () => {
    try {
      const res = await api.get("/eb");
      setEBs(res.data);
    } catch (err) {
      toast.error("Failed to fetch EBs");
    }
  };

  const handleDelete = async (id: string) => {
    try {
      await api.delete(`/eb/${id}`);
      toast.success("EB deleted");
      fetchEBs();
    } catch (err) {
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-gray-600 text-sm mt-1">
                    Create and manage your organization's executive board members
                  </p>
                </div>
              </div>
              <Button
        ...
        {ebs.length === 0 && (
          <div className="text-center py-16">
            ...
            <p className="text-gray-600 mb-6">
              Get started by creating your first EB member.
            </p>
            <Button
              onClick={() => {
                setSelected(null);
